...
const Detail = () => {
	...
	useEffect(() => {
		document.title = (movie?.title || movie?.name) && !isLoading ? movie.title || movie.name : 'tMovies';
		return () => {
		...
		<>
			<section className='w-full' style={backgroundStyle}>
				...
				>
					...
					>
						<m.h2 variants={fadeDown} className={cn(mainHeading, ' md:max-w-[420px]')}>
							{title || name}
						</m.h2>
						...
						<m.p variants={fadeDown} className={paragraph}>
							<span>{overview.length > 280 ? `${show ? overview : `${overview.slice(0, 280)}...`}` : overview}</span>
							<button
								type='button'
								className={cn(
