...
export const Section = memo(({ title, category, className, type, id, showSimilarShows }: SectionProps) => {
	...
	const errorMessage = useMemo(() => (isError ? getErrorMessage(error) : ''), [error, isError]);
		...
		<section className={sectionStyle} ref={ref}>
			...
			<div className='xs:min-h-[250px] min-h-[216px]'>
				{isLoading ? (
					...
					<Error error={String(errorMessage)} className='xs:h-[250px] h-[216px] text-[18px]' />
				) : (
